...
<xsl:stylesheet version="2.0"
	...
	<xsl:template match="snapshot">
		...
		<rdf:RDF xmlns:rdfs="http://www.w3.org/2000/01/rdf-schema#"
			...
			xmlns:frbr="http://purl.org/vocab/frbr/core#" xmlns:bibo="http://purl.org/ontology/bibo/"
			xmlns:swc="http://data.semanticweb.org/ns/swc/ontology#" xmlns:swrc="http://swrc.ontoware.org/ontology#"
			...
			<xsl:for-each select="submissions/submission">
				...
				<xsl:if
					test="/snapshot/decisions/decision/id[text()=$decisionID]/../accepted='true'">
					<rdf:Description>
						<xsl:attribute name="rdf:about">
							<xsl:value-of
							select="concat($baseConference, 'paper/research/', number/text())" />
		 				</xsl:attribute>
						<rdf:type>
							<xsl:attribute name="rdf:resource">
		 						<xsl:value-of
								select="'http://swrc.ontoware.org/ontology#InProceedings'" />
		 					</xsl:attribute>
						</rdf:type>
						<dc:title>
						...
						<bibo:authorList>
							<xsl:attribute name="rdf:resource">
		 						<xsl:value-of
								select="concat($baseConference, 'paper/research/', number/text(), '/authorList')" />
		 					</xsl:attribute>
						</bibo:authorList>
						...
						<swrc:year>
							<xsl:value-of select="$conferenceYear"></xsl:value-of>
						</swrc:year>
						<swrc:month>
							<xsl:value-of select="$conferenceMonth"></xsl:value-of>
						</swrc:month>
        ...
        <xsl:value-of select="concat($baseConference,'/proceedings')" />
			...
			<!--rdf:Description rdf:about="http://data.semanticweb.org/conference/ld4ie/2015"> 
				...
				<swc:hasAcronym> <xsl:value-of select="baseConference" /> </swc:hasAcronym> 
				</rdf:Description -->
			...
			<!-- rdf:Description>
				...
				<rdf:type
					rdf:resource="http://data.semanticweb.org/ns/swc/ontology#ConferenceEvent" />
				<swc:completeGraph>
					<xsl:attribute name="rdf:resource">
